...
export async function sendCicloPendienteEmail(toEmail, supervisorNombre, funcion, dias) {
  await resend.emails.send({
    ...
    html: baseHtml(`
      ...
      <p style="color:#555">Hace <strong>${dias} días</strong> que <strong>${funcion}</strong> no tiene un ciclo de relevamiento abierto, así que su operativo no recibe preguntas nuevas.</p>
